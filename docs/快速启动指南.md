...
# 直接启动后端（查看详细日志）
cd d:\files\demo\classaudio
D:\Anaconda\envs\deepl\python.exe -m uvicorn src.api.server:app --host 0.0.0.0 --port 8000 --log-level debug
```
...
```bash
cd d:\files\demo\classaudio
D:\Anaconda\envs\deepl\python.exe -m unittest discover -s tests -v
```
